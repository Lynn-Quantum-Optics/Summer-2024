...
This file contains the Laser class, which interfaces with the software loaded onto the Nucleo-32 chip being used to monitor the laser's status. It works very simmilarly to the CCU class.
...
class Laser:
    ''' Class for Laser Interfacing.

    Parameters
    ----------
    port : str
        Serial port of the Nucleo-32 chip (e.g. 'COM1' on windows, or '/dev/ttyUSB1' on unix).
    baud : int
    ...
    # +++ class variables +++
    UPDATE_PERIOD = 0.1 # seconds, from device software
    CHANNEL_KEYS = ['PhotoDiodeVoltage', 'LaserDiodeCurrent', 'Temperature'] # from device software
    NUM_CHANNEL = len(CHANNEL_KEYS)
    TERMINATION_SEQ = b'\xff\xff\xff\xff' # from device software
    ...
    def __init__(self, port:str, baud:int, raw_data_csv=None, ignore:'list[str]'=[]) -> None:
        ...
        self._listening_process = mp.Process(target=Laser._listen_and_plot, args=(conn_to_self, self.port, self.baud, raw_data_csv, ignore))
        self._listening_process.start()

    # +++ CLEANUP +++
    
    def shutdown(self) -> None:
        ''' Closes the connection to the Nucleo-32 chip by terminating the listening process. '''
        # send close notice
    ...
    def _flush(conn, one=False) -> None:
        ''' Flushes the input buffer from the Nucleo-32. 
        
        Parameters
        ----------
        conn : serial.Serial
            Serial connection to the Nucleo-32.
        one : bool, optional
        ...
        # read all data until the next terminatin sequence
        seen = b''
        while seen[-len(Laser.TERMINATION_SEQ):] != Laser.TERMINATION_SEQ:
            seen += conn.read(1)
