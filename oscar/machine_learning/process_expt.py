...
def analyze_rhos(filenames, settings=None):
    ...
    for i, file in tqdm(enumerate(filenames)):
        if settings is None:
            try:
                trial, rho, _, Su, rho_actual, fidelity, purity, eta, chi, angles = get_rho_from_file(file, verbose=False)
            except:
                trial, rho, _, Su, rho_actual, fidelity, purity, angles = get_rho_from_file(file, verbose=False)
                eta, chi = None, None
        else:
            try:
...
def make_plots_E0(dfname):
    ...
    chi_ls = np.linspace(-90, 0, 1000)


    ax[0,0].scatter(chi_45, W_T_45, label='$W_T$', color='navy')
    ax[0,0].scatter(chi_45, W_AT_45, label='$W_{AT}$', color='blue')
    ax[0,0].errorbar(chi_45, W_expt_45, yerr = W_unc_45, label='$W_{expt}$', fmt='s', color='slateblue')

    ax[0,0].scatter(chi_45, Wp_AT_45, label="$W_{AT}'$", color='red')
    ax[0,0].scatter(chi_45, Wp_T_45, label="$W_T'$", color='crimson')
    ax[0,0].errorbar(chi_45, Wp_expt_45, yerr = Wp_unc_45, label="$W_{expt}'$", fmt='s', color='salmon')

    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_T_45), color='navy')
    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_T_45), color='crimson')
    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_AT_45), color='blue')
    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_AT_45), color='red')
    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_expt_45), color='slateblue')
    ax[0,0].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_expt_45), color='salmon')
    ...
    ax[0,1].scatter(chi_60, W_T_60, label='$W_T$', color='navy')
    ax[0,1].scatter(chi_60, W_AT_60, label='$W_{AT}$', color='blue')
    ax[0,1].errorbar(chi_60, W_expt_60, yerr = W_unc_60, label='$W_{expt}$', fmt='s', color='slateblue')

    ax[0,1].scatter(chi_60, Wp_T_60, label="$W_T'$", color='crimson')
    ax[0,1].scatter(chi_60, Wp_AT_60, label="$W_{AT}'$", color='red')
    ax[0,1].errorbar(chi_60, Wp_expt_60, yerr = Wp_unc_60, label="$W_{expt}'$", fmt='s', color='salmon')
    ...
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_T_60), color='navy')
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_T_60), color='crimson')
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_AT_60), color='blue')
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_AT_60), color='red')
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_W_expt_60), color='slateblue')
    ax[0,1].plot(chi_ls, sinsq2(chi_ls, *popt_E0_Wp_expt_60), color='salmon')
    ...
    plt.tight_layout()
    plt.savefig(join(DATA_PATH, 'exp_witnesses_E0.pdf'))
    plt.show()
    ...
    def make_settings_plots():
        ...
        for i, angle in enumerate(UV_HWP):
            ax[0].plot(angle, 1, label='(%.3g, %.3g, %.3g)' % (np.rad2deg(eta_total[i]), np.rad2deg(chi_total[i]), trials[i]), marker='o', color=colors[i])
        ...
        for i, angle in enumerate(QP):
            ax[1].plot(angle, 1, label='(%.3g, %.3g, %.3g)' % (np.rad2deg(eta_total[i]), np.rad2deg(chi_total[i]), trials[i]), marker='o', color=colors[i])
        ax[1].set_title('QWP settings')
        ax[1].set_xlim(min(QP)-0.1, max(QP)+0.1)
        ax[1].legend()

        # plot B HWP settings
        for i, angle in enumerate(B_HWP):
            ax[2].plot(angle, 1, label='(%.3g, %.3g, %.3g)' % (np.rad2deg(eta_total[i]), np.rad2deg(chi_total[i]),  trials[i]), marker='o', color=colors[i])
        ...
        plt.savefig(join(DATA_PATH, 'settings.pdf'))
...
if __name__ == '__main__':
    ...
    # # analyze rho files
    # analyze_rhos(filenames, settings)

    make_plots_E0('rho_analysis.csv')