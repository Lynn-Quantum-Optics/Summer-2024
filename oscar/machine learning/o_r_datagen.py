...
def gen_dataset(size, savepath):
    ...
    df_3 = pd.DataFrame({'HH':[], 'VV':[], 'HV':[], 'min_eig':[]})
    df_5 = pd.DataFrame({'HH':[], 'VV':[], 'HV':[], 'DD':[], 'AA':[], 'min_eig':[]})
    df_6 = pd.DataFrame({'HH':[], 'VV':[], 'HV':[], 'DD':[], 'RR':[], 'LL':[], 'min_eig':[]})
    df_12 = pd.DataFrame({'DD':[], 'AA':[], 'DL':[], 'AR':[], 'DH':[], 'AV':[], 'LL':[], 'RR':[], 'LH':[], 'RV':[], 'HH':[], 'VV':[], 'min_eig':[]})
    df_15 = pd.DataFrame({'DD':[], 'AA':[], 'DL':[], 'AR':[], 'DH':[], 'AV':[], 'LL':[], 'RR':[], 'LH':[], 'RV':[], 'HH':[], 'VV':[], 'DR':[], 'DV':[], 'LV':[], 'min_eig':[]})
    ...
    for j in trange(size):
        ...
        df_3 = df_3.append({'HH':HH, 'VV':VV, 'HV':HV,'min_eig':min_eig}, ignore_index=True)

        DD = compute_proj(D, D, M0)
        AA = compute_proj(A, A, M0)
        df_5 = df_5.append({'HH':HH, 'VV':VV, 'HV':HV,'DD':DD, 'AA':AA, 'min_eig':min_eig}, ignore_index=True)

        RR = compute_proj(R,R, M0)
        LL = compute_proj(L, L, M0)
        df_6 = df_6.append({'HH':HH, 'VV':VV, 'HV':HV,'DD':DD, 'RR':RR,'LL': LL,'min_eig':min_eig}, ignore_index=True)
        ...
        df_12 = df_12.append({'DD':DD, 'AA':AA, 'DL':DL, 'AR':AR, 'DH':DH, 'AV':AV, 'LL':LL, 'RR':RR, 'LH':LH, 'RV':RV, 'HH':HH, 'VV':VV, 'min_eig':min_eig}, ignore_index=True)

        DR = compute_proj(D,R, M0)
        DV = compute_proj(D,V, M0)
        LV = compute_proj(L,V, M0)
        df_15 = df_15.append({'DD':DD, 'AA':AA, 'DL':DL, 'AR':AR, 'DH':DH, 'AV':AV, 'LL':LL, 'RR':RR, 'LH':LH, 'RV':RV, 'HH':HH, 'VV':VV, 'DR':DR, 'DV':DV, 'LV':LV, 'min_eig':min_eig}, ignore_index=True)
