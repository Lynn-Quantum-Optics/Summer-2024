...
if __name__ == '__main__':
    ...
    m.shutdown()
    '''
    df = Manager.load_data('AQWP_sweep2.csv')
    '''
    # fit the function
    ...
    # plotting
    analysis.plot_func('quadratic', params, df['A_QWP'], color='b', linestyle='dashed', label=f'Fit Function')
    analysis.plot_errorbar(df['A_QWP'], df['C4'], ms=0.1, fmt='ro', label='Data')
    plt.xlabel('Alice\'s QWP Angle (deg)')
    plt.ylabel('Count Rate (#/s)')
    plt.legend()
    plt.title(f'Fit=${params[1].n:.3f}(x-{params[0].n:.3f})^2 + {params[2].n:.3f}$')
    # plt.title(f'Fit=${params[2].n:.3f}[\\cos^2({params[1].n:.3f})+\\cos(2\\cdot{params[1].n:.3f})\\sin^2(2(\\theta{params[0].n:.3f}))/2]+{params[3].n:.3f}$')
