...
def random_unit_vector_gaussian(dim):
    x = np.random.randn(dim)*np.exp(1j*np.pi*np.random.rand(dim))
...
N = 10000
states = np.array([random_unit_vector_gaussian(2) for _ in range(N)])
...
fig.canvas.manager.set_window_title('method1_pure_bloch_sphere')
plt.show()
...
fig.canvas.manager.set_window_title('method1_pure_histograms')
plt.show()
